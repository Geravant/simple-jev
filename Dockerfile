...
#   TEMPLATE       prompt template: v1 (default) or the compact v2c
...
ENV MODEL=/repository DTYPE=bfloat16 MAX_MODEL_LEN=8192 MAX_IMAGES=4 PORT=8000 TEMPLATE=v1 \
...
CMD ["sh", "-c", "exec simple-jev --model \"$MODEL\" ${MODEL_ALIAS:+--model-alias \"$MODEL_ALIAS\"} --device cuda:0 --dtype \"$DTYPE\" --max-model-len \"$MAX_MODEL_LEN\" --max-images \"$MAX_IMAGES\" --template-version \"$TEMPLATE\" --host 0.0.0.0 --port \"$PORT\""]
