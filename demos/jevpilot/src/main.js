import { pilotFetch, preparePilotRequest, MODELS, MODEL, setModel, pricingFor } from "./simple-jev-api.js";
import "./style.css";
import {
  createIcons,
  Braces,
...
<div class="world-picker"><label for="world-select">Change map</label><select id="world-select" aria-label="Change map"><option value="city">Skyline City</option><option value="town">Small town</option><option value="highway">Interstate 08</option></select><button id="new-world" title="Generate a new layout of this map" aria-label="New map layout">${icon("rotate-cw")}<span>New layout</span></button><a id="github-link" href="https://github.com/standardagents/jevpilot" target="_blank" rel="noopener noreferrer" aria-label="View original JevPilot source" title="View on GitHub">${icon("github")}</a></div><div class="model-picker"><label for="model-select">AI model</label><select id="model-select">${MODELS.map(model => `<option value="${model.id}">${model.label}</option>`).join("")}</select></div></header>
<div class="simple-jev-api-badge">PUBLIC DEMO · No key · 2k context / 4 RPS<br><small>Live driving · WASD to drive · J for autopilot.</small></div><div class="navigation-hud"><div class="navigation-card glass"><span id="turn-icon">${icon("arrow-up")}</span><div><strong id="next-maneuver">Continue straight</strong><span id="turn-distance"></span></div><span class="nav-divider"></span><span id="remaining"></span><button id="map-toggle" aria-label="Toggle route map" aria-pressed="true" title="Hide route map">${icon("map")}</button></div>
...
function inspectRequest(state) {
  const { request, fixed } = preparePilotRequest(state);
...
$("model-select").onchange = (event) => {
  // Invalidate any previous model's pending driving plan without pausing physics.
  setPilot(false);
  setModel(event.target.value);
  updateCostTooltip(pricingFor());
  toast("Model changed. Engage Jev to drive with it.");
};
function updateCostTooltip(pricing) {
  ...
  tooltips.set(
    ...
    `${playCredits ? "Remaining from your one-time $0.25 allowance. " : ""}${averages}Session total uses each request’s model price. Current model: $${pricing.input_per_million.toFixed(2)}/M input. This public demo is free. Developer-plan beta pricing may change; see featherless.ai for current pricing.`,
  );
}
...
async function decide() {
  ...
  try {
    ...
    const res = await pilotFetch("/api/decide", {
        method: "POST",
...
pilotFetch("/api/status", { credentials: "same-origin" })
  .then((r) => r.json())
  .then((data) => {
